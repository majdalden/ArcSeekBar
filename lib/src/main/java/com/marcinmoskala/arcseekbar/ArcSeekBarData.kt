...
internal data class ArcSeekBarData(
    ...
    private val alphaRad: Float = bound(zero, acos((r - height).toDouble() / r).toFloat(), 2 * pi)
    val arcRect: RectF =
    ...
    val progressSweepRad = if (maxProgress == 0) zero else bound(
        zero,
        progressValue.toFloat() / maxProgress * 2 * alphaRad,
        2 * pi
    )
    val progressSweepAngle: Float = progressSweepRad / 2 / pi * 360F
    val thumbX: Int =
        (r * cos(alphaRad + Math.PI / 2 - progressSweepRad).toFloat() + circleCenterX).toInt()
    val thumbY: Int =
        (-r * sin(alphaRad + Math.PI / 2 - progressSweepRad).toFloat() + circleCenterY).toInt()

    fun progressFromClick(x: Float, y: Float, thumbHeight: Int): Int? {
        if (y > height + dy * 2) return null
        val distToCircleCenter = sqrt(
            (circleCenterX - x.toDouble()).pow(2.0) + (circleCenterY - y.toDouble()).pow(
                2.0
            )
        )
        if (abs(distToCircleCenter - r) > thumbHeight) return null
        val innerWidthHalf = width / 2
        val xFromCenter = bound(-innerWidthHalf, x - circleCenterX, innerWidthHalf).toDouble()
        val touchAngle = acos(xFromCenter / r) + alphaRad - Math.PI / 2
